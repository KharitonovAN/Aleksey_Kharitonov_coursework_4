...
class JSONSaver:
    ...
    def read_from_json():
        ...
        with open(f'vacancies.json', encoding='utf-8') as file:
            data = json.load(file)
        for item in data:
            for vacancy in item:
                ...
                Vacancy.vacancy_obj_list.append(Vacancy(vacancy_name, area, salary_from, salary_to,
                                                        address, published, requirement, responsibility, employment,
                                                        platform, vacancy_url))
